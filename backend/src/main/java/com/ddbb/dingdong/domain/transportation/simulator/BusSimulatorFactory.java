...
import com.ddbb.dingdong.domain.transportation.simulator.subscription.source.BusSimulator;
...
public class BusSimulatorFactory {
    ...
    /**
     * @param start : 버스 출발 시간
     * 주어진 Segment 리스트를 기반으로 정해진 UNIT_SECOND 마다의 버스 위치를 계산하여
     * 각 단위초마다 버스 위치를 시뮬레이트 하는 BusSimulator를 만듭니다.
     * **/
    public BusSimulator create(LocalDateTime start) {
        List<RouteSegment> segments = segmentProvider.getRouteSegments(start);
